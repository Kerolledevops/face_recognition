...
def consumer(num_cam ,buffer_dict ,data_dict, buffer_size=BUFFER_SIZE):
    ...
    :param num_cam: camera number.
    """
     topic = "{}_{}".format(PREDICTION_TOPIC_PREFIX,num_cam)
     msg_stream = KafkaConsumer(topic ,group_id=GROUP_ID,
                                bootstrap_servers=[BOOTSTRAP_SERVERS],
                                auto_offset_reset=AUTO_OFFSET_RESET,
                                value_deserializer=lambda value: json.loads(
                                    value.decode()
                                ))
     try:   #start consuming msg stream
         while True:
             frame_number = 0
             original_frame ,predicted_frame = bytes(0)
             start_buffer_consumption = False
             try:
                 raw_messages = msg_stream.poll(timeout_ms=12, max_records= 12)

                 for topic_partition , msgs in raw_messages.items():  # Get the predicted Object, JSON with frame and meta info about the frame
                     for msg  in msgs:
                         prediction_obj = msg.value
                         frame_num = int(prediction_obj["frame_num"]) #frame cam num

                         original_png , predicted_png = get_png(prediction_obj) #extract images from the prediction message
                         heappush(buffer_dict[num_cam],frame_num) #heap buffer to maintaint the order : only frame number
                         data_dict[num_cam][frame_num] = (original_png.tobytes(),predicted_png.tobytes())
                         print("\r[CAM {}][PART 1][BUFFER] Pushed: {} {}/{}".format(num_cam, prediction_obj["frame_num"],
                                                                len(buffer_dict[num_cam]),
                                                                buffer_size),end="")

                         if len(buffer_dict[num_cam]) >= buffer_size:
                             start_buffer_consumption=True


                    if start_buffer_consumption:
                        print("[CAM{}][PART 2] YIELD".format(num_cam))
                        last_frame_num = frame_number

                        if len(buffer_dict[num_cam]):
                            frame_number = heappop(buffer_dict[num_cam])
                            print(frame_number)
                            original_frame, predicted_frame = data_dict[num_cam][frame_number]
                            ...
                            print(
                                "[CAM {}] STREAM ENDED AT FRAME {}".format(num_cam, last_frame_num)
                            )
                            yield (
                                b"--frame\r\n"
                                b"Content-Type"
         ...
         print("Closing sttream")
         msg_stream.close()



def consume_buffer(num_cam, buffer_dict ,data_dict ,event_threads, lock ,buffer_size=BUFFER_SIZE):
    """Generator to yield frames from the respective camera. Threaded Concept.
       ...
       """
    print(
        "\n[CAM {}][FASTAPI] Pushing to FastAPI".format(num_cam)
    )
    frame_number = 0 # init variables
    ...
    while True:
        ...
        if len(buffer_dict[num_cam]):
            ...
            yield (
                b"--frame\r\n"
                b"Conntent-Type: image/png\r\n\r\n"+predicted_frame+ b"\r\n\r\n"
            )
        else:
            lock.release()
            print(
                "[CAM{}] NO STREAM AFTER FRAME {}".format(num_cam, last_frame_num)
            )
            yield (
                b"--frame\r\n"
                b"Content-Type: image/png\r\n\r\n"+predicted_frame+b"\r\n\r\n"
...
# B. 2.
def populate_buffer(msg_stream, num_cam, buffer_dict, data_dict, event_threads, buffer_size=180):
    ...
    :param num_cam: camera number, used to access respective buffer, or data
    """
    ...
    try:
        ...
        while True:
            try:
                ...
                for topic_partition, msgs in raw_messages.items(): # Get the predicted Object, JSON with frame and meta info about the frame    
                    for msg in msgs:
                        prediction_obj = msg.value # Get the predicted Object, JSON with frame and meta info about the frame   
                        frame_num = int(prediction_obj["frame_num"]) # extract images from the prediction message
                        original_png, predicted_png = get_png(prediction_obj)

                        # HEAP BUFFER TO MAINTAIN THE ORDER: ONLY FRAME NUMBERS ARE PUSHED
                        heappush(buffer_dict[num_cam], frame_num)
                        # DATA DICT: TO COLLECT REAL FRMAES
                        data_dict[num_cam][frame_num] = (original_png.tobytes(), predicted_png.tobytes())
                        # print log
                        print("\r[CAM {}][BUFFER] Pushed: {} {}/{}".format(num_cam, prediction_obj["frame_num"],
                                                                           len(buffer_dict[num_cam]), buffer_size),
                              end="")

                        # as soon as buffer is full for the first time, start consuming/display event on flask
                        if len(buffer_dict[num_cam]) == buffer_size and not event_threads[num_cam].is_set():
                            print("\n[CAM {}][BUFFER] Starting Consuming loop..".format(num_cam))
                            event_threads[num_cam].set()
